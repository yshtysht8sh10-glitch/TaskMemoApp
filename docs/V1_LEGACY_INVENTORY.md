...
- The user approved dropping orphan `ipa-morning`; the separate 124-Node repaired source validates with zero issues/loss. Comparison with the 120-document snapshot is non-identical (10 identical, 4 Export-only, 0 Firestore-only, 110 field differences), so an explicit authoritative-source decision is required before migration/rehearsal.
