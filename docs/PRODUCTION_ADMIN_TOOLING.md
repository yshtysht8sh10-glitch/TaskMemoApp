...
The approved canary operation JSON contains `uid`, `migrationId`, `opId`, `nodeId`, `expectedRevision`, the complete next `record`, and complete `operation`. It must be independently reviewed and hashed. Before commit, the CLI reads the current winner, requires the expected revision, uses the normal V2 revision evaluator to prove that the operation deterministically produces exactly the reviewed next record, and binds the atomic commit to the observed Firestore update time. Executing its write form crosses the POINT OF NO RETURN.
