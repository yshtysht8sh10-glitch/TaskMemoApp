...
type Props = { configured: boolean; environment: TaskMemoEnvironment | null; configurationError: string | null; user: User | null; status: FirebaseSyncStatus; error: string | null; onSignIn: (email: string, password: string) => Promise<void>; onSignUp: (email: string, password: string) => Promise<void>; onSignOut: () => Promise<void> };
...
export function SyncAccountPanel({ configured, environment, configurationError, user, status, error, onSignIn, onSignUp, onSignOut }: Props) {
