...
describe('Firebase Node boundary', () => {
  ...
  it('同一日のRoutine完了は端末間で重複せず日付キー1件へ統合する', () => { const local = { ...memo('a', new Date(30)), routineHistory: { '2026-09-17': '2026-09-17T01:00:00.000Z' } }; const remote = { ...memo('a', new Date(20)), routineHistory: { '2026-09-17': '2026-09-17T02:00:00.000Z' } }; const merged = mergeNodesByUpdatedAt([local], [remote])[0]; expect(merged.type === 'memo' && Object.keys(merged.routineHistory ?? {})).toEqual(['2026-09-17']); });
  it('新しい端末更新の完了取消tombstoneを古い完了履歴より優先する', () => { const cancelled = { ...memo('a', new Date(30)), routineHistory: { '2026-09-17': null } }; const completed = { ...memo('a', new Date(20)), routineHistory: { '2026-09-17': '2026-09-17T02:00:00.000Z' } }; const merged = mergeNodesByUpdatedAt([cancelled], [completed])[0]; expect(merged.type === 'memo' && merged.routineHistory?.['2026-09-17']).toBeNull(); });
