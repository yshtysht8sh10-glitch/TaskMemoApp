...
export function nodeFromFirestore(
  id: string,
...
export function mergeNodesByUpdatedAt(local: Node[], remote: Node[]) {
  ...
  for (const node of local) {
    ...
    const chosen =
      (!!node.purgedAt && !cloud.purgedAt) ||
      (!!node.purgedAt === !!cloud.purgedAt &&
        node.updatedAt.getTime() > cloud.updatedAt.getTime())
        ? node
        : cloud;
    if (node.type === "memo" && cloud.type === "memo" && !chosen.purgedAt) {
      if ((chosen as MemoNode).memoType === "idea") {
        merged.set(node.id, chosen);
        continue;
      }
...
export function withRemoteTombstones(
  nodes: Node[],
  remote: Node[],
  now = new Date(),
) {
...
export function applyRemoteDeletionsAsTombstones(
  nodes: Node[],
