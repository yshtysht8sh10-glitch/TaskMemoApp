...
describe('Node backup', () => {
  ...
  it('壊れたJSONとschemaVersionを拒否する', () => { expect(() => parseNodeBackup('{')).toThrow(/JSON/); expect(() => parseNodeBackup('{"schemaVersion":3,"exportedAt":"2026-01-01T00:00:00Z","nodes":[]}')).toThrow(/対応/); });
  ...
  it('schema 2で常設メモを往復しschema 1を常設メモなしで読み込む', () => {
    const pinnedNote = { body: 'shared pinned note', updatedAt: date };
    expect(parseTaskMemoBackup(serializeTaskMemoBackup([category, memo], pinnedNote, date))).toEqual({ nodes: [category, memo], pinnedNote });
    expect(parseTaskMemoBackup(serializeNodeBackup([category], date))).toEqual({ nodes: [category], pinnedNote: null });
  });
});
