...
export function normalizeLegacyRanks(nodes: Node[]) {
  ...
  for (const parentId of parents) {
    ...
    const valid = siblings.every((node, index) => {
      if (index > 0 && node.sortKey === siblings[index - 1].sortKey)
        return false;
      try {
        generateKeyBetween(node.sortKey, null);
        return true;
      } catch {
        return false;
      }
    ...
    if (!valid) {
      ...
      const replacements = new Map(
        siblings.map((node, index) => [node.id, keys[index]]),
      );
      result = result.map((node) =>
        replacements.has(node.id)
          ? { ...node, sortKey: replacements.get(node.id)! }
          : node,
      );
    }
  }
