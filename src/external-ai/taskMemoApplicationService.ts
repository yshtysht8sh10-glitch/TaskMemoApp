...
const memoView = (memo: MemoNode, nodes: readonly Node[]): MemoView => {
...
function resolveMemo(nodes: readonly Node[], selector: MemoSelector, includeDeleted = false) {
  ...
  if (matches.length > 1) {
    throw new ExternalAiError(
      ...
      matches.map((memo) => memoView(memo, nodes)),
    );
  }
  return matches[0];
}
...
export class TaskMemoApplicationService {
  constructor(
    ...
    private readonly createId: () => string = () => `memo-${crypto.randomUUID()}`,
  ) {}

  async listMemos(user: AuthenticatedTaskMemoUser, input: ListMemosInput = {}) {
    const nodes = await this.repository.read(requireUser(user));
    ...
    return nodes.filter((node): node is MemoNode => node.type === 'memo'
      ...
      .map((memo) => memoView(memo, nodes));
  }

  async getMemo(user: AuthenticatedTaskMemoUser, selector: MemoSelector) {
    const nodes = await this.repository.read(requireUser(user));
    return memoView(resolveMemo(nodes, selector, true), nodes);
  }
  ...
  async createMemo(user: AuthenticatedTaskMemoUser, input: CreateMemoInput) {
    ...
    return this.repository.transact(uid, (nodes) => {
      ...
      return { nodes: next, result: memoView(resolveMemo(next, { memoId: id }), next) };
  ...
  async updateMemo(user: AuthenticatedTaskMemoUser, input: UpdateMemoInput) {
    ...
    return this.repository.transact(uid, (nodes) => {
      const memo = resolveMemo(nodes, input);
      assertActiveParent(nodes, input.parentId);
      ...
      return { nodes: next, result: memoView(resolveMemo(next, { memoId: memo.id }), next) };
  ...
  async completeMemo(user: AuthenticatedTaskMemoUser, selector: MemoSelector) {
    const uid = requireUser(user);
    return this.repository.transact(uid, (nodes) => {
      const memo = resolveMemo(nodes, selector);
      if (memo.status === 'completed') return { nodes, result: memoView(memo, nodes) };
      const next = completeMemoOperation(nodes, memo.id, this.now());
      return { nodes: next, result: memoView(resolveMemo(next, { memoId: memo.id }), next) };
    }, { operation: 'complete_memo' });
  }
  ...
  async deleteMemo(user: AuthenticatedTaskMemoUser, selector: MemoSelector) {
    ...
    return this.repository.transact(uid, (nodes) => {
      ...
      return { nodes: next, result: memoView(resolveMemo(next, { memoId: memo.id }, true), next) };
  ...
  async listCategories(user: AuthenticatedTaskMemoUser): Promise<CategoryView[]> {
    ...
    return nodes.filter((node) => node.type === 'category' && node.deletedAt === null && !node.purgedAt)
      .map((node) => ({ id: node.id, title: node.title, parentId: node.parentId, sortKey: node.sortKey }));
  }
}
