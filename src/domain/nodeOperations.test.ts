...
import { canContainMemo, canMoveNode, compareNodes, compareSortKeys, completeMemo, createNode, duplicateMemo, hardDeleteNode, moveMemos, moveNode, restoreMemo, restoreNode, softDeleteNode, tryMoveNode, visibleNodes } from './nodeOperations';
...
describe('node operations', () => {
  ...
  it('ルーティーン直下へMemoを作成・移動でき、移動だけでは繰り返さない', () => { const root = { ...base('routine', 'category', null, 'z0'), categoryKind: 'routineRoot' as const }; const source = [...tree(), root]; expect(canContainMemo(source, 'routine')).toBe(true); expect(createNode(source, 'memo', { title: 'new', parentId: 'routine' }, now, 'new').find((node) => node.id === 'new')).toMatchObject({ parentId: 'routine', repeatRule: null }); const moved = moveNode(source, 'm2', 'routine').find((node) => node.id === 'm2'); expect(moved).toMatchObject({ parentId: 'routine', repeatRule: null }); expect(moveMemos(source, ['m2'], 'routine', now).find((node) => node.id === 'm2')).toMatchObject({ parentId: 'routine', repeatRule: null }); });
  it('常設ルーティーンCategoryは移動・削除できない', () => { const root = { ...base('routine', 'category', null, 'z0'), categoryKind: 'routineRoot' as const }; const source = [...tree(), root]; expect(canMoveNode(source, 'routine', 'a')).toBe(false); expect(softDeleteNode(source, 'routine', true, now)).toBe(source); expect(hardDeleteNode(source, 'routine', now)).toBe(source); });
