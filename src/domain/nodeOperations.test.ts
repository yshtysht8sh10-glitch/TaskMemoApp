...
describe('node operations', () => {
  ...
  it('ルーティーン直下へのMemo作成・移動を拒否し周期Category配下は許可する', () => { const root = { ...base('routine', 'category', null, 'z0'), categoryKind: 'routineRoot' as const }; const daily = { ...base('daily', 'category', 'routine', 'a0'), categoryKind: 'routineDaily' as const }; const source = [...tree(), root, daily]; expect(canContainMemo(source, 'routine')).toBe(false); expect(canContainMemo(source, 'daily')).toBe(true); expect(() => createNode(source, 'memo', { title: 'invalid', parentId: 'routine' }, now, 'invalid')).toThrow(/直下/); expect(() => moveNode(source, 'm2', 'routine')).toThrow(/移動/); expect(moveNode(source, 'm2', 'daily').find((node) => node.id === 'm2')?.parentId).toBe('daily'); expect(() => moveMemos(source, ['m2'], 'routine', now)).toThrow(/直下/); });
