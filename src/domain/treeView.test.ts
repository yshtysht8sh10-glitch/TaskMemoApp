...
describe('tree row layout metadata', () => {
  ...
  it('root Memoを仮想の無所属配下に置き、完了表示を切り替える', () => { const active = memo('active', null, 'a0'); const done = { ...memo('done', null, 'a1'), status: 'completed' as const, completedAt: now }; expect(flattenVisibleNodes([active, done], new Set([UNASSIGNED_GROUP_ID])).map((row) => row.node.id)).toEqual([UNASSIGNED_GROUP_ID, 'active']); expect(flattenVisibleNodes([active, done], new Set([UNASSIGNED_GROUP_ID]), true).map((row) => row.node.id)).toEqual([UNASSIGNED_GROUP_ID, 'active', 'done']); });
});
