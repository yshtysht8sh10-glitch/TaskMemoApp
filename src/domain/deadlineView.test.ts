...
describe('deadline view', () => {
  ...
  it('非表示bucketを定義済みfallbackへ移しても期限を変更しない', () => { const memo = base('memo', new Date(2026, 8, 13, 11)); const original = memo.type === 'memo' ? memo.dueAt : null; const groups = deadlineGroups([memo], now, new Set(['pm']), 'amPm'); expect(groups[0].memos[0]).toBe(memo); expect(memo.type === 'memo' && memo.dueAt).toBe(original); });
